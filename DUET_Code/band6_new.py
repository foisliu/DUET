...
####
hdu = fits.open('/Users/liuxihe/Desktop/DUET_figure_2/sgrb1off_south_cont_selfcal.commonbeam.image.fits')[0]
hdu_pbcor = fits.open('/Users/liuxihe/Desktop/DUET_figure_2/sgrb1off_south_cont_selfcal.commonbeam.image.pbcor.fits')[0]
...
if (not os.path.isfile('sgrb1off_dendro_cores_deg.dat')) or owcat:
    ...
    d = Dendrogram.compute(hdu.data, min_value=4*rms, min_delta=1*rms, min_npix=beam)#阈值
    ...
    with open('sgrb1off_dendro_cores.dat', 'w') as newfile:
        for row in list(zip(np.arange(len(radecs)),radecs,np.round(ra*206265,decimals=-1),fluxes,masses,densities)):
            print("{:>4} {: >26} {:.0f} {:.2f} {:.2f} {:.1e}".format(*row), file=newfile)
    # This is for codes, coordinates are in degrees
    with open('sgrb1off_dendro_cores_deg.dat', 'w') as newfile:
        for row in list(zip(list(cat['_idx']),radec_deg,ra*206265,fluxes,masses,densities)):
            print("{: >3} {: >30} {:.0f} {:.2f} {:.2f} {:.2e}".format(*row), file=newfile)
...
if (not os.path.isfile('sgrb1off_dendro_mask_band6.fits')) or owcat:
    mask = np.zeros(hdu.data.shape, dtype=bool)
    for leaf in leaves_repro:
        mask = mask | leaf.get_mask()
    
    newmask = np.array(mask, dtype=int)
    fits.writeto('/Users/liuxihe/Desktop/sgrb1off_dendro_mask_band6.fits', newmask, hdu.header, overwrite=True)

    #index_only3 = np.loadtxt('/Users/liuxihe/Desktop/我的/天协/科创数据/only_band6.dat', usecols=(0,), dtype=int)
    #leaves_only3 = []
    #for i, leaf in enumerate(leaves_repro):
    #    if i in index_only3:
    #        leaves_only3.append(leaf)
    #mask_only3 = np.zeros(hdu.data.shape, dtype=bool)
    #for leaf in leaves_only3:
    #    mask_only3 = mask_only3 | leaf.get_mask()
    #newmask_only3 = np.array(mask_only3, dtype=int)
    #fits.writeto('/Users/liuxihe/Desktop/我的/天协/科创数据/sgrb1off_dendro_mask_only6.fits', newmask_only3, hdu.header, overwrite=True)
    ...
    mask_hdu = fits.open('sgrb1off_dendro_mask_band6.fits')[0]
...
f1.show_colorscale(cmap='cubehelix_r', vmax=2, vmin=2e-2, aspect='equal', stretch='log')
f1.show_contour(mask_hdu, colors='red', linewidths=1.0)
...
f1.colorbar.set_axis_label_text('$I_\mathrm{3\,mm}$ (mJy/beam)')

f1.add_label(0.03, 0.93, r'Sgr B1-off Cores', relative=True, weight='black', size='large', ha='left', zorder=50)
...
f1.save('/Users/liuxihe/Desktop/sgrb1off_cores_band6.pdf',dpi=150)
